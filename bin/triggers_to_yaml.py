...
def parse_triggers_txt(filepath):
    ...
    for line in lines:
        ...
        if event_match:
            ...
            if float(angle) >= 720 / get_crank_divider(current_trigger['metadata']['operationMode'], channel, current_trigger['metadata']['isSecondWheelCam']):
                continue
            ...
            if state == "0":
                ...
                if last is None or last['times'] > 1:
                    continue
            ...
            try:
                ...
                continue;
            penultAngle = (penult['angle'] + (penult['every'] * (penult['times'] - 1)))
            if (math.isclose(ult['width'], penult['width'], rel_tol=1e-4, abs_tol=1e-4)
                and (penult['times'] == 1 or math.isclose(ult['angle'] - penultAngle, penult['every'], rel_tol=1e-4, abs_tol=1e-4))):
                penult['every'] = ult['angle'] - penultAngle
                penult['times'] += 1
...
def finish_trigger(current_trigger):
    for channel in current_trigger['wrappedFall'].keys():
        ...
        try:
            ...
            continue;
        penultAngle = (penult['angle'] + (penult['every'] * (penult['times'] - 1)))
        if (ult['width'] == penult['width']
            and (penult['times'] == 1 or ult['angle'] - penultAngle == penult['every'])):
            penult['every'] = ult['angle'] - penultAngle
            penult['times'] += 1
            current_trigger['events'] = current_trigger['events'][:-1]
...
def process_cam_trigger(f, secondWheelCam):
    ...
    for e in f:
        if e['channel'] == 0:
            e['angle'] = e['angle'] / 2
            e['width'] = e['width'] / 2
            if e['times'] > 1: e['every'] = e['every'] / 2
            p.append(e)
        else:
            if secondWheelCam:
                e['angle'] = e['angle'] / 2
    ...
    for trigger in triggers:
        ...
        if trigger['events']:
            ...
            for idx, event in enumerate(events):
                ...
                if event["times"] > 1: lines.append(f'      times: {event["times"]}')
                if event["times"] > 1: lines.append(f'      every: {format_value(event["every"])}')
