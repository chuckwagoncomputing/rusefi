...
void TriggerWaveform::setEvents(const ComplexTriggerSignal* events, size_t length) {
	...
	// We need the last trigger defined to be at angle 360 or 720 (see `full` below)
	ComplexTriggerSignal wrapped = {TriggerWheel::T_PRIMARY, 0.0f, 0.0f, 0, 0.0f};
  ...
  for (size_t i = 0; i < length; i++) {
    ComplexTriggerSignal e = events[i];
		for (int t = 0; t < e.times; t++) {
			angle_t angle = e.angle + (t * e.every);
			// Wrap this rise event
			if (angle < 0.0001f) {
				wrapped = {e.wheel, full, e.width, e.times, e.every};
			} else {
				addPendingFall(pendingFall, angle);
				addEvent(angle / full, TriggerValue::RISE, e.wheel);
			}
			// Events too close together are not allowed by MultiChannelStateSequence
			if (e.width < 0.0001f) {
				// Some minimum is required for trigger stimulation. I'm not sure what it is, but this seems to work.
				e.width = 0.5f;
			}
			// Store the pending fall, regardless of whether the rise event was wrapped.
			// This means the trigger pattern will begin with a fall and end with a rise.
			pendingFall[(int)e.wheel] = {e.wheel, angle + e.width, 0.0f, 1, 0.0f};
		}
	}
	addPendingFall(pendingFall, full);
	if (wrapped.times > 0) {
		addEvent(wrapped.angle / full, TriggerValue::RISE, wrapped.wheel);
	}
}
...
void TriggerWaveform::addPendingFall(ComplexTriggerSignal* pendingFall, angle_t angle) {
	...
	if (pendingFall[0].times > 0 && pendingFall[1].times > 0) {
	...
	for (int i : indices) {
		if (pendingFall[i].times > 0 && pendingFall[i].angle <= angle) {
			addEvent(pendingFall[i].angle / full, TriggerValue::FALL, pendingFall[i].wheel);
			// Reset
			pendingFall[i].times = 0;
		}
	}
}
