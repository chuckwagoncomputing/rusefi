...
public class TriggerWheelInfo {
    ...
    private static TriggerWheelInfo readTriggerWheelInfo(int id, Map<String, Object> triggerMap) {
        ...
        // Convert repeated signals into simple signals (still ComplexTriggerSignals, but with `times = 1` and `every = 0.0`)
        ...
        for (int s = 0; s < complexSignals.size(); s++) {
            ...
            for (int e = 0; e < signal.getTimes(); e++) {
                Double angle = signal.getAngle() + (signal.getEvery() * e);
    ...
    private static int getCrankDivider(String operationMode) {
    ...
    // but it is different to this one because this one is
    // meant for wheels that are always defined within 0-360.
    private static int getCycleDuration(String operationMode) {
        switch (operationMode) {
        case "OM_NONE":
        case "FOUR_STROKE_CAM_SENSOR":
        case "FOUR_STROKE_CRANK_SENSOR":
    ...
    // Rotate the list such that index `index` is at angle zero.
    public <T extends TriggerEvent> void rotateSignals(List<T> rotatedSignals, int index) {
        Collections.rotate(rotatedSignals, -index);
        double baseAngle = rotatedSignals.get(0).getAngle();

        for (T signal : rotatedSignals) {
            double newAngle = signal.getAngle() - baseAngle;

            if (newAngle < 0) {
                newAngle += getCycleDuration(operationMode);
            }

            signal.setAngle(newAngle);
        }
    }
    ...
    public List<TriggerSignal> getSignals() {
        ...
        for (int i = 0; i < getCrankDivider(operationMode); i++) {
            for (int s = 0; s < signals.size(); s++) {
                ComplexTriggerSignal signal = signals.get(s);
                // ...with an offset.
                Double angle = signal.getAngle() + i * (360 / getCrankDivider(operationMode));
        ...
        rotateSignals(multSignals, syncPoint);
        return multSignals;
    }
    ...
    public List<ComplexTriggerSignal> getComplexSignals() {
        ...
        while (csi.hasNext()) {
            ...
            if (ult != null) {
                Double ultAngle = (ult.getAngle() + (ult.getEvery() * (ult.getTimes() - 1)));
                // If the last signal had the same width, is on the same wheel, and the gap matches,
                // increment the last signal's `times` field and drop this signal from the list.
                if (Math.abs(ult.getWidth() - current.getWidth()) < 0.0001f
                    && ult.getWaveIndex() == current.getWaveIndex()
                    && (ult.getTimes() == 1 || Math.abs((current.getAngle() - ultAngle) - ult.getEvery()) < 0.0001f)) {
                    ult.setEvery(current.getAngle() - ultAngle);
                    ult.setTimes(ult.getTimes() + 1);
                    csi.remove();
                } else {
                    ult = current;
                }
        ...
        // Rotate the list such that the angle of the first element is 0.
        rotateSignals(expandedSignals, 0);
