...
TEST(nissan, vq_vvt) {
	...
	while ((head = engine->scheduler.getHead()) != nullptr) {
		...
		ASSERT_NEAR(99, tc->vvtPosition[0][0], EPS2D) << "queueIndex=" << queueIndex;
    	queueIndex++;
	}
	ASSERT_EQ(queueIndex, 434) << "Total queueIndex=" << queueIndex;

	ASSERT_TRUE(tc->vvtState[1][0].getShaftSynchronized());

	ASSERT_NEAR(99, tc->vvtPosition[0][0], EPS2D);
	ASSERT_NEAR(99, tc->vvtPosition[1][0], EPS2D);
